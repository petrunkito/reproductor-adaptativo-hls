...
/*
videoElement: es un elemento de de tipo video(<video>)
config: recibe un objeto que puede modificar los siguientes parametros(timeSpace, keyQuality, keyVolume, lastData)
cb(): ejecutamos esta funcion para que el codigo de otro modulo(que esta dentro de la funcion "cb"), pueda acceder sin complicaciones al objeto "hlsElement"
*/
async function start(videoElement, config = {}, cb) {
    try {
        ...
        videoElement.addEventListener("ended",()=>_emit('finish'))//?cada que llegue al final el video se hallan descargados unos o todos los fragmentos emitiremos el evento "finish"
        ...
        async function volume(num) {
            if (num && (num >= 0 && num <= 1)) {//?si el usuario modifica el volumen y ese numero esta en el rango de 0 a 1, tambien lo agrega al localStorage
                ...
                localStorage.setItem(hlsElement.keyVolume, 1)
            }
        }
        ...
        //!las configuracion y asignaciones de listeners, y la cadena de acciones posteriores
        await _init(async () => {
        ...
        async function _initHlsElement() {
            ...
            //?ejecutamos aqui el callback, para que el codigo que esta dentro de la funcion "cb" en otro mudulo, pueda acceder
            //?al objeto hlsElement y a los metodos(on, volume, getTimeRanges, changeResolution)
            await cb({ hlsElement, on, volume, getTimeRanges, changeResolution })
        ...
        let obj = {
            "playing": [],//?se dispara cada vez que el video se preproduce, esto en un periodo de un segundo
            "add": [],//?se dispara cuando se agrega uno o varios fragmentos al sourceBuffer
