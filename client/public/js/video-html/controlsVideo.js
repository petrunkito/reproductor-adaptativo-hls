...
async function init(videoElement, hls, playPauseSelector, volumeSelector, volumeRangeSelector,
    ...
    let statePlayPauseButton = ["play", "pause"]
    ...
    videoElement.addEventListener('timeupdate', e=>{
        $playPauseButton.dataset.play = "play"
        $playPauseButton.src = "/img/play.png"
    }, {once:true})


    async function getTime(time, sum=0) {
        let tiempo = time || 0//?si el time llega a ser un valor nulo o un "NaN" le asignamos 0
    ...
    async function playPauseVideo(e) {
        ...
        target.dataset.play = is==="pause"?"play":"pause"
    }
