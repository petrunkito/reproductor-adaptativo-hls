...
async function deleteFile(idFile){
    try{
        ...
        if (!response.ok) throw new Error('An error occurred while deleting the video')

        let result = (await response.json()).ok
        return result
    }catch(err){
        console.log(`deleteFile api error: ${err.message}`)
        return false
    }
}
...
export default {
    getFile, getFiles, deleteFile, getPlaylist, getPlaylists, postUploader, getFragment
}